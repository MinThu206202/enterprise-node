...
import type { ITransactionContext } from "../../services/database/ITransactionContext.js";
import type { ICommandHandler } from "../../bus/ICommand.js";
import type { RemoveRoleFromUserCommand } from "./RemoveRoleFromUserCommand.js";
...
export class RemoveRoleFromUserCommandHandler implements ICommandHandler<RemoveRoleFromUserCommand, void> {
  ...
  private async handle(input: RemoveRoleFromUserInput): Promise<void> {
    ...
    await this.unitOfWork.execute(async (tx) => {
      await tx.userRoleRepository.remove(input.userId, input.roleId);
      ...
      await tx.outboxRepository.create({
        type: EVENT_TYPES.USER_ROLE_REMOVED,
        payload: {
          eventId,
          messageId,
          eventType: EVENT_TYPES.USER_ROLE_REMOVED,
          eventVersion: 1,
